...
export default function MitraTambahTransaksi({ namaVenueProps }) {
    ...
    const handlePost = async (e) => {
        ...
        if (jamFilter.length === 0) {
            if (jadwalPesan.length == 0) {
                alert('Tidak ada Jadwal yang dinonaktifkan')
            }
            else {
                let nama = ''
                let email = ''
                ...
                if (data.success) {
                    // reset the fields
                    alert('Jadwal Berhasil Dinonaktifkan')
            ...
            <div className='mt-3'>
                <form onSubmit={handlePost}>
                    ...
                    <div className='row'>
                        ...
                        <button type='submit' className='btn btn-danger text-white mt-3' disabled={(session) ? (false) : (true)} onClick={() => checkValue()}>Nonaktifkan Jadwal</button>
