...
export default function MitraTambahTransaksi({ namaVenueProps }) {
    ...
    const { namaVenue, namaLapangan, idTransaksi, jadwalMainReq } = router.query
    const jadwalMain = JSON.parse(jadwalMainReq)
    ...
    const { data: data, error } = useSWR(`/api/editjadwaldb?&namaVenueReq=${namaVenue}&namaLapanganReq=${namaLapangan}&tglMainReq=${tglMain}`, fetcher, { refreshInterval: 1000 })
    ...
    const handlePost = async (e) => {
        ...
        if (jamFilter.length === 0) {
            if (jadwalPesan.length != jadwalMain.length) {
                alert('Jumlah jam tidak sesuai dengan jam yang dipesan')
            }
            else {
                await fetch('/api/editjadwaldb', {
                    method: 'PUT',
                    headers: {
                ...
                return router.push('/mitra/data-transaksi');
                // let nama = ''
                // let email = ''
    ...
    return (
        <div className="container mt-4">
            <h1 className='mb-3 mt-4'>Edit Jadwal - {infoLapangan.namaLapangan}</h1>
                ...
                <div className="container-xxl card p-3 shadow-lg">
                    <div className="row">
                        <div className="px-md-5 p-3 col-md-12 align-items-start justify-content-center" >
                            <h1><b>Jadwal Saat Ini</b></h1>
                            <h3 ><b>Lapangan:</b>&nbsp;{namaLapangan}</h3>
            ...
            <div className='mt-3'>
                <form onSubmit={handlePost}>
                    ...
                    <div className='row'>
                        <h4><b>Jadwal yang akan diedit:</b></h4>
                        ...
                        <button type='submit' className='btn btn-fill text-white mt-3' disabled={(session) ? (false) : (true)} onClick={() => checkValue()}>Edit Jadwal Main</button>
